...
def eval(element):
    ...
    preds, gts, depth_preds, depth_gts, depth_thres, eval_frames, gt_dir, pred_dir = element
    ...
    gts = [np.array(Image.open(os.path.join(gt_dir, name))) for name in gts if '_instanceTrainIds.png' in name]

    abs_rel = 0
    if depth_thres > 0:
        depth_preds = [np.array(Image.open(os.path.join(pred_dir, name)))
...
def main(eval_frames, pred_dir, gt_dir, depth_thres):
    ...
    all_files = []
    for seq_id in gts:
        depth_preds_seq_id = depth_preds[seq_id] if depth_thres > 0 else None
        all_files.append([preds[seq_id], gts[seq_id],
                          depth_preds_seq_id, depth_gts[seq_id], depth_thres, eval_frames, gt_dir, pred_dir])
    N = mp.cpu_count() // 2 + 1
    ...
    rq = tp_per_class / np.maximum(tp_per_class + 0.5 * fn_per_class + 0.5 * fp_per_class, epsilon)
    pq = sq * rq
    print('k={}, lambda={}, result:\n'.format(eval_frames, depth_thres),
         ...
         '{:.2f}  {:.2f}  {:.2f}'.format(
            ...
            pq[:11].mean() * 100))
    ret = dict()
    ret['averages'] = (pq.mean()*100, pq[11:].mean()*100, pq[:11].mean()*100)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth_thres', type=float, default=-1)
    ...
    gt_dir = 'video_sequence-cityscapes/val'

    main(eval_frames, pred_dir, gt_dir, depth_thres)
