...
def load_cityscapes_panoptic(image_dir, gt_dir, gt_json, meta):
    ...
    assert os.path.exists(
        gt_json
    ), gt_json+"not exists. Please run `python cityscapesscripts/preparation/createPanopticImgs.py` to generate label files."  # noqa
    ...
    for image_file, label_file, segments_info in files:
        sem_label_file = (
            image_file.replace("leftImg8bit", "gtFine").split("_gtFine.")[0] + "_gtFine_labelTrainIds.png"
        )
...
if __name__ == "__main__":
    ...
    dataset = DatasetCatalog.get("cityscapes_fine_panoptic_train_separated")
    print(dataset[-1])
    print(len(dataset))
