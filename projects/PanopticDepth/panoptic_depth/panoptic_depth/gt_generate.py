...
class GenerateGT(object):
    ...
    def assign_multi_scale(self, gt_instances, gt_semantics, image_shape, inst_outsize, feat_shape):
        ...
        if hasattr(gt_instances, 'gt_masks'):
            ...
            bit_masks, mass_centers, scales = GenerateGT.decode_bitwise_mask(common_stride=self.common_stride,
                                                                             bit_masks=gt_instances.gt_masks.tensor,
                                                                             device=self.device,
                                                                             use_mass=use_mass,
                                                                             )
    ...
    def _sem_assign(self, gt_semantic, feat_shape): 
        ...
        if 0:#self.depth_on:
            gt_scoremap = F.interpolate(gt_semantic.unsqueeze(0), size=feat_shape, 
                          mode='nearest')[0]
        else:
            gt_scoremap = F.interpolate(gt_semantic.unsqueeze(0), size=feat_shape, 
                          mode='bilinear', align_corners=False)[0]
    ...
    def generate(self, batched_input, images, features, inst_feat, test_only=False):
        ...
        if self.depth_on:
            gt_depths = []
            for x in batched_input:
                gt_depth = torch.zeros(1, *inst_shape, device=self.device)

                gt_depths_raw = GenerateGT.decode_bitwise_mask(
                                    common_stride=self.common_stride,
                                    bit_masks=x["depth"].unsqueeze(0),
                                    device=self.device,
                                    use_mass=False,
                                    sampling_strategy='nearest',
                                )[0] 
                _, h, w = gt_depths_raw.shape
                gt_depth[:, :h, :w] = gt_depths_raw
                gt_depths.append(gt_depth)

            gt_depths = torch.stack(gt_depths, dim=0)
            gt_dict.update({"depth": gt_depths})

        return gt_dict

    @staticmethod
    def decode_bitwise_mask(common_stride, bit_masks, device, use_mass=False,sampling_strategy='bilinear'):
        """
