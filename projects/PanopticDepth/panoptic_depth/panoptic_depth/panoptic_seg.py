...
class PanopticFCN(nn.Module):
    ...
    def loss_single_level(self, pred_center, pred_region, pred_weights, \
        ...
        weighted_values, guided_index = \
            torch.topk(guidence.reshape(*guided_inst.shape[:2], -1), k=self.weighted_num, dim=-1)
        ...
        idx_feat_st = idx_feat_st.sum(dim=(-1,-2), keepdims=True) / \
                      torch.clamp(gt_sem_masks.reshape(-1, 1, *weight_shape[-2:]).sum(dim=(-1,-2), keepdims=True), min=1)
